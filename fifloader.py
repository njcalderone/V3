...
# Copyright Red Hat
#
# This file is part of os-autoinst-distri-v3gateway.
#
# os-autoinst-distri-v3gateway is free software; you can redistribute it
...
"""This is an openQA template loader/converter for FIF, the v3gateway Intermediate Format. It reads
...
The loader will automatically set the group_name for each job template based on v3gateway-specific
logic which we previously followed manually when creating job templates (e.g. it is set to 'v3gateway
PowerPC' for compose tests run on the PowerPC arch); thus this loader is not really generic but
specific to v3gateway conventions. This could possibly be changed (e.g. by allowing the logic for
...
def generate_job_templates(products, profiles, testsuites):
    ...
    for (name, suite) in testsuites.items():
        ...
        for (profile, prio) in suite['profiles'].items():
            ...
            # x86_64 compose
            jobtemplate['group_name'] = 'v3gateway'
            ...
            if jobtemplate['machine_name'] == 'ppc64le':
                if 'updates' in product['flavor']:
                    jobtemplate['group_name'] = "v3gateway PowerPC Updates"
                else:
                    jobtemplate['group_name'] = "v3gateway PowerPC"
            elif jobtemplate['machine_name'] in ('aarch64', 'ARM'):
                if 'updates' in product['flavor']:
                    jobtemplate['group_name'] = "v3gateway AArch64 Updates"
                else:
                    jobtemplate['group_name'] = "v3gateway AArch64"
            elif 'updates' in product['flavor']:
                # x86_64 updates
                jobtemplate['group_name'] = "v3gateway Updates"
